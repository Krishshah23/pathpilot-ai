        ...
        <div className="space-y-4">
          ...
          {resumeHistory.length > 1 && (
            <div className="card p-5">
              <p className="text-[10px] font-bold uppercase tracking-wider text-faint mb-3">Score History</p>
                ...
                {resumeHistory.slice().reverse().map((h, i) => {
                  ...
                  return (
                    <div key={h._id} className="flex flex-col items-center gap-1 flex-1" title={`${new Date(h.createdAt).toLocaleDateString()} — ${h.healthScore}/100`}>
                      <span className="text-[9px] font-bold text-faint">{h.healthScore}</span>
                      <div
                        className="w-full rounded-t-md transition-all"
                        style={{ height: `${barH}px`, backgroundColor: isLatest ? '#2B4C3F' : 'var(--line-border)' }}
                      />
                    </div>
                  );
                })}
              </div>
              <p className="text-[10px] text-faint mt-2">{resumeHistory.length} versions uploaded</p>
            </div>
          )}
        </div>
